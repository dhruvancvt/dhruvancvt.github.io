...
// To change the password: run `sha256sum` or use https://emn178.github.io/online-tools/sha256.html
...
// Re-number visible rows on filter
function renumber() {
    const visible = [...rows].filter(r => !r.classList.contains('hidden'));
    visible.forEach((r, i) => {
        const idx = r.querySelector('.wr-index');
